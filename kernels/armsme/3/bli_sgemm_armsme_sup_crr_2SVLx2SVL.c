...
)
{
    ...
    for ( uint64_t trow = 0; trow < SVL; trow += 1 )
    {
        ...
        svfloat32_t z0 = svread_ver_za32_m( svundef_f32(), pg, 0, trow );
        svfloat32_t z1 = svread_ver_za32_m( svundef_f32(), pg, 1, trow );
        svfloat32_t z2 = svread_ver_za32_m( svundef_f32(), pg, 2, trow );
        svfloat32_t z3 = svread_ver_za32_m( svundef_f32(), pg, 3, trow );
        ...
        float *c_ptr_0 = &c_[result_tile_TL_corner + trow * cs_c];
        float *c_ptr_1 = &c_[result_tile_TL_corner + SVL + trow * cs_c];
        float *c_ptr_2 = &c_[result_tile_BL_corner + trow * cs_c];
        float *c_ptr_3 = &c_[result_tile_BL_corner + SVL + trow * cs_c];
