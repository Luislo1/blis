...
)
{
    ...
    // Manually aligned stack buffers
    float packed_A_raw[2 * SVL * k + 16];
    float *packed_A = (float *)(((uintptr_t)packed_A_raw + 63) & ~63);
    float packed_B_raw[2 * SVL * k + 16];
    float *packed_B = (float *)(((uintptr_t)packed_B_raw + 63) & ~63);
    ...
    for (uint64_t kk = 0; kk < k; kk += 2 * SVL)
    {
        for (uint64_t trow = 0; trow < SVL; trow += 4)
        {
            float *row_ptr_top = b_orig + trow * cs_b + kk;
            float *row_ptr_bot = b_orig + (trow + SVL) * cs_b + kk;

            svfloat32x2_t zp01 = svld1_f32_x2( pg_c, row_ptr_top + 0 * cs_b );
            svfloat32x2_t zp23 = svld1_f32_x2( pg_c, row_ptr_top + 1 * cs_b );
            svfloat32x2_t zp45 = svld1_f32_x2( pg_c, row_ptr_top + 2 * cs_b );
            svfloat32x2_t zp67 = svld1_f32_x2( pg_c, row_ptr_top + 3 * cs_b );

            svfloat32x2_t zp89 = svld1_f32_x2( pg_c, row_ptr_bot + 0 * cs_b );
            svfloat32x2_t zpAB = svld1_f32_x2( pg_c, row_ptr_bot + 1 * cs_b );
            svfloat32x2_t zpCD = svld1_f32_x2( pg_c, row_ptr_bot + 2 * cs_b );
            svfloat32x2_t zpEF = svld1_f32_x2( pg_c, row_ptr_bot + 3 * cs_b );
        ...
        for (uint64_t tcol = 0; tcol < SVL; tcol += 4)
        {
            ...
            float *pack_ptr_L = &packed_B[(kk + tcol + 0) * 2 * SVL];
            float *pack_ptr_R = &packed_B[(kk + tcol + 0 + SVL) * 2 * SVL];
            
            svst1_f32_x2(pg_c, pack_ptr_L, svcreate2(svget4(zq0, 0), svget4(zq2, 0)));
            svst1_f32_x2(pg_c, pack_ptr_R, svcreate2(svget4(zq1, 0), svget4(zq3, 0)));

            pack_ptr_L = &packed_B[(kk + tcol + 1) * 2 * SVL];
            pack_ptr_R = &packed_B[(kk + tcol + 1 + SVL) * 2 * SVL];
            
            svst1_f32_x2(pg_c, pack_ptr_L, svcreate2(svget4(zq0, 1), svget4(zq2, 1)));
            svst1_f32_x2(pg_c, pack_ptr_R, svcreate2(svget4(zq1, 1), svget4(zq3, 1)));

            pack_ptr_L = &packed_B[(kk + tcol + 2) * 2 * SVL];
            pack_ptr_R = &packed_B[(kk + tcol + 2 + SVL) * 2 * SVL];
            
            svst1_f32_x2(pg_c, pack_ptr_L, svcreate2(svget4(zq0, 2), svget4(zq2, 2)));
            svst1_f32_x2(pg_c, pack_ptr_R, svcreate2(svget4(zq1, 2), svget4(zq3, 2)));

            pack_ptr_L = &packed_B[(kk + tcol + 3) * 2 * SVL];
            pack_ptr_R = &packed_B[(kk + tcol + 3 + SVL) * 2 * SVL];
    ...
    float *pack_b_ptr = packed_B;

    for (uint64_t k_ = 0; k_ < k; k_ += 4)
    {
        // Steps 0 and 1 Loads
        svfloat32x4_t zL01 = svld1_f32_x4(pg_c, pack_a_ptr);
        svfloat32x4_t zR01 = svld1_f32_x4(pg_c, pack_b_ptr);

        // Step 0 Outer Products
        svmopa_za32_m( 0, pg, pg, svget4(zL01, 0), svget4(zR01, 0));
        svmopa_za32_m( 1, pg, pg, svget4(zL01, 1), svget4(zR01, 0));
        svmopa_za32_m( 2, pg, pg, svget4(zL01, 0), svget4(zR01, 1));
        svmopa_za32_m( 3, pg, pg, svget4(zL01, 1), svget4(zR01, 1));

        // Step 1 Outer Products
        svmopa_za32_m( 0, pg, pg, svget4(zL01, 2), svget4(zR01, 2));
        svmopa_za32_m( 1, pg, pg, svget4(zL01, 3), svget4(zR01, 2));
        svmopa_za32_m( 2, pg, pg, svget4(zL01, 2), svget4(zR01, 3));
        svmopa_za32_m( 3, pg, pg, svget4(zL01, 3), svget4(zR01, 3));

        // Steps 2 and 3
        svfloat32x4_t zL23 = svld1_f32_x4(pg_c, pack_a_ptr + 4 * SVL);
        svfloat32x4_t zR23 = svld1_f32_x4(pg_c, pack_b_ptr + 4 * SVL);

        svmopa_za32_m( 0, pg, pg, svget4(zL23, 0), svget4(zR23, 0));
        svmopa_za32_m( 1, pg, pg, svget4(zL23, 1), svget4(zR23, 0));
        svmopa_za32_m( 2, pg, pg, svget4(zL23, 0), svget4(zR23, 1));
        svmopa_za32_m( 3, pg, pg, svget4(zL23, 1), svget4(zR23, 1));

        svmopa_za32_m( 0, pg, pg, svget4(zL23, 2), svget4(zR23, 2));
        svmopa_za32_m( 1, pg, pg, svget4(zL23, 3), svget4(zR23, 2));
        svmopa_za32_m( 2, pg, pg, svget4(zL23, 2), svget4(zR23, 3));
        svmopa_za32_m( 3, pg, pg, svget4(zL23, 3), svget4(zR23, 3));
